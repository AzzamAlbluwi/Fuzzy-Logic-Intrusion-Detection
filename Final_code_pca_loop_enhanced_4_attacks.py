...
fuzzers_traffic_data = drop_columns(fuzzers_traffic, non_numerical_cols)
...
print(f"Number of fuzzers data points: {fuzzers_traffic.shape[0]}")

num_rows_to_select = 3000  # Replace this with the desired number of rows to select
num_testing = 9000

# Using .iloc to select the first 'num_rows_to_select' rows from `normal_traffic`
normal_traffic = normal_traffic_data.iloc[:9000]
normal_traffic_test = normal_traffic_data.iloc[9000:9000 + 9000]

generic_traffic = generic_traffic_data.iloc[:num_rows_to_select]
generic_traffic_test = generic_traffic_data.iloc[num_rows_to_select:
                                                 num_rows_to_select + num_rows_to_select]

exploits_traffic = exploits_traffic_data.iloc[:num_rows_to_select]
exploits_traffic_test = exploits_traffic_data.iloc[
    num_rows_to_select:num_rows_to_select + num_rows_to_select]

fuzzers_traffic = fuzzers_traffic_data.iloc[:num_rows_to_select]
fuzzers_traffic_test = fuzzers_traffic_data.iloc[num_rows_to_select:
                                                 num_rows_to_select + num_rows_to_select]
...
traffic = pd.concat([normal_traffic, generic_traffic,
                    exploits_traffic, fuzzers_traffic], ignore_index=True)
traffic_test = pd.concat([normal_traffic_test, generic_traffic_test,
                         exploits_traffic_test, fuzzers_traffic_test], ignore_index=True)
...
print("-----------------------------------------------------")
print(f"Best True Positive Percentage (TPP): {best_tpp:.2f}%")
print(f"Best False Positive Percentage (FPP): {best_fpp:.2f}%")
print(f"Best True Negative Percentage (TNP): {best_tnp:.2f}%")
print(f"Best False Negative Percentage (FNP): {best_fnp:.2f}%")
print(f"Precision: {precision:.4f}")
print(f"Recall (True Positive Rate): {recall:.4f}")
print(f"F-score: {f_score:.4f}")
